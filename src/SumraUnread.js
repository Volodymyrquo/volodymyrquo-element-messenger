...
import { haveTileForEvent } from "./components/views/rooms/SumraEventTile";
...
export function eventTriggersUnreadCount(ev) {
    if (
        ...
        ev.getType() == "m.call.answer" ||
        ev.getType() == "m.call.hangup"
    ) {
        return false;
    } else if (
        ev.getType() == "m.room.message" &&
        ev.getContent().msgtype == "m.notify"
    ) {
        return false;
    } else if (
        ev.getType() == "m.room.aliases" ||
        ev.getType() == "m.room.canonical_alias"
    ) {
        return false;
