...
import MultiInviter from "./utils/SumraMultiInviter";
...
import InviteDialog, {
    KIND_DM,
    KIND_INVITE,
} from "./components/views/dialogs/SumraInviteDialog";
...
export function showCommunityRoomInviteDialog(roomId, communityName) {
    const CommunityPrototypeInviteDialog = sdk.getComponent(
        "views.dialogs.CommunityPrototypeInviteDialog"
    );

    Modal.createTrackedDialog(
        "Invite Users to Community",
        "",
        CommunityPrototypeInviteDialog,
        { communityName, roomId },
        /*className=*/ null,
        /*isPriority=*/ false,
        /*isStatic=*/ true
    );
...
export function showCommunityInviteDialog(communityId) {
    ...
    if (chat) {
        const name =
            CommunityPrototypeStore.instance.getCommunityName(communityId);
        showCommunityRoomInviteDialog(chat.roomId, name);
    } else {
        throw new Error(
            "Failed to locate appropriate room to start an invite in"
        );
    }
}
...
export function inviteUsersToRoom(roomId, userIds) {
    return inviteMultipleToRoom(roomId, userIds)
        .then((result) => {
