...
import {IdentityProviderBrand, IIdentityProvider, ISSOFlow} from "matrix-react-sdk/src/Login";
...
const getIcon = (brand: IdentityProviderBrand | string) => {
    switch (brand) {
        case IdentityProviderBrand.Apple:
            return require(`matrix-react-sdk/res/img/element-icons/brands/apple.svg`);
        case IdentityProviderBrand.Facebook:
            return require(`matrix-react-sdk/res/img/element-icons/brands/facebook.svg`);
        case IdentityProviderBrand.Github:
            return require(`matrix-react-sdk/res/img/element-icons/brands/github.svg`);
        case IdentityProviderBrand.Gitlab:
            return require(`matrix-react-sdk/res/img/element-icons/brands/gitlab.svg`);
        case IdentityProviderBrand.Google:
            return require(`matrix-react-sdk/res/img/element-icons/brands/google.svg`);
        case IdentityProviderBrand.Twitter:
            return require(`matrix-react-sdk/res/img/element-icons/brands/twitter.svg`);
        default:
            return null;
    }
}
...
const SSOButtons: React.FC<IProps> = ({matrixClient, flow, loginType, fragmentAfterLogin, primary}) => {
    ...
    if (providers.length < 2) {
        return <div className="sumra-Button">
            <SSOButton
                matrixClient={matrixClient}
                loginType={loginType}
    ...
    return <div className="sumra-Button">
        { chunk(providers, size).map(chunk => (
            <div key={chunk[0].id} className="sumra-Button">
                { chunk.map(idp => (
                    <SSOButton
                        key={idp.id}
                        matrixClient={matrixClient}
